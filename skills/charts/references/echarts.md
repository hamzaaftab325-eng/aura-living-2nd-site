...
function baseOption(theme, title, subtitle) {
  return {
    ...
    textStyle: {
      fontFamily: "system-ui, SimHei, sans-serif",
      color: theme.text,
    },
    title: {
      text: title,
  ...
  series: [
    {
      ...
      label: {
        ...
        color: "#6B7280",
        formatter: (p) =>
          p.dataIndex === 2 ? "{hl|" + p.value + "}" : p.value,
...
const option = {
  ...
  yAxis: [
    { type: "category", data: products, gridIndex: 0, ...cleanAxis(THEME) },
  ],
  series: [
    {
      type: "bar",
