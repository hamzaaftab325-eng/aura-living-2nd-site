...
function drawAllLines() {
  ...
  const leftSel =
    ".left-side > .branch, .left-branches > .left-branch, .left-branches > div";
  ...
  leftBranches.forEach((branch) => {
    ...
    processChildren(
      bNode,
      branch.querySelector(":scope > .children"),
      null,
      1,
      "left",
    );
  ...
  const rightSel =
    ".right-side > .branch, .branches > .branch, .right-branches > .right-branch";
...
body {
  background: #ffffff;
  font-family:
    -apple-system, BlinkMacSystemFont, "PingFang SC", "SimHei", sans-serif;
}
#mindmap {
  padding: 60px;
