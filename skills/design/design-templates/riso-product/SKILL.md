...
:root {
  --hanli:
    "白舟隷書教漢", "HakushuReisho", "Yuji Mai", "Yuji Boku", "ZCOOL XiaoWei",
    "STLiti", "华文隶书", "LiSu", "SimLi", Georgia, serif;
}
/* 若要 CDN 兜底 */
...
.riso-shade {
  ...
  background:
    radial-gradient(
      ellipse 55% 65% at 18% 25%,
      rgba(255, 63, 142, 0.55) 0%,
      transparent 60%
    ),
    radial-gradient(
      ellipse 50% 55% at 82% 75%,
      rgba(34, 64, 255, 0.5) 0%,
      transparent 60%
    ),
    radial-gradient(
      ellipse 30% 35% at 65% 18%,
      rgba(255, 210, 58, 0.4) 0%,
      transparent 70%
    );
  mix-blend-mode: multiply;
}
