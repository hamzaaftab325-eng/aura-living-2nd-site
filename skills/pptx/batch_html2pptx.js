...
async function buildPptxFromDir(slidesDir, outputPath, options = {}) {
  ...
  console.log(
    `  ${htmlFiles.length} slides → ${outputPath} (concurrency=${concurrency})`,
  );
  ...
  async function worker() {
    while (true) {
      ...
      try {
        const { warnings } = await html2pptx(htmlFile, pptx, {
          fontConfig,
          slide,
        });
...
async function main() {
  ...
  const filteredArgs = args.filter((arg, i) => {
    ...
    if (arg === "--concurrency" || arg.startsWith("--concurrency="))
      return false;
    if (concIdx !== -1 && !args[concIdx].includes("=") && i === concIdx + 1)
      return false;
    return true;
  });
