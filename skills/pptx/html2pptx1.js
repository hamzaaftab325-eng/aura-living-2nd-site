...
function isNarrowBodyTextForPptCompensation(el) {
  ...
  return (
    txt.length >= 35 && txt.length <= 160 && fs <= 14 && el.position.w < 1.7
  );
}
...
function getAdjustedTextPosition(el, slideWidthIn) {
  ...
  const widthFactor = isRotatedText
    ? 0
  ...
  const finalW = isRotatedText
    ? adjustedW
    : Math.min(adjustedW, MAX_TEXT_WIDTH_IN);
...
function checkElementBounds(slideData, sw, sh) {
  ...
  for (const el of slideData.elements) {
    ...
    const rawP = textTypes.has(el.type)
...
function isShortAutoNoWrapText(el) {
  ...
  if (
    compactTxt.length === 0 ||
    compactTxt.length > COMPENSATION.AUTO_SHORT_TEXT_THRESHOLD
  ) {
    return false;
  }
  const fs = Number(el.style?.fontSize) || 16;
...
function calculateWidthCompensation(el, slideWidthIn) {
  ...
  if (txt.length > 0 && txt.length <= 5)
    f += COMPENSATION.VERY_SHORT_TEXT_EXTRA;
  if (/^[\d\s\.\,\-\/\+\%\$\#\@\!\?\:\;\(\)\[\]]+$/.test(txt))
...
function isIconOnlyTextNode(text = "") {
  ...
  if (hasCJKCharsNode(compact) || /[A-Za-z0-9０-９]/.test(compact))
    return false;
  ...
  for (const el of slideData.elements) {
    ...
    if (typeof el.text === "string") {
      ...
      if (isIconOnlyTextNode(trimmed)) {
        ...
        hasEmojiCharsNode(trimmed) &&
        Array.from(trimmed).length <= 4
      ) {
        el.style.fontFace = cfg.symbol;
      }
      if (shouldUseNumericEmphasisText(trimmed))
        el.style.fontFace = cfg.emphasis;
      ...
      for (const run of el.text) {
        ...
        if (shouldUseNumericEmphasisText(trimmed))
          run.options.fontFace = cfg.emphasis;
      }
    }
  }
}
...
async function materializeObjectFitImages(slideData, tmpDir) {
  ...
  const tasks = slideData.elements.map(async (el) => {
    ...
    if (
      !el.position ||
      !Number.isFinite(el.position.w) ||
      !Number.isFinite(el.position.h)
    )
      return;
    ...
    try {
      ...
      if (el.objectFit === "cover") {
        ...
        const left = clampNumber(
          Math.round((resizedW - targetW) * pos.x),
          0,
          resizedW - targetW,
        );
        const top = clampNumber(
          Math.round((resizedH - targetH) * pos.y),
          0,
          resizedH - targetH,
        );
        await sharp(input)
        ...
        const left = clampNumber(
          Math.round((targetW - resizedW) * pos.x),
          0,
          targetW - resizedW,
        );
        const top = clampNumber(
          Math.round((targetH - resizedH) * pos.y),
          0,
          targetH - resizedH,
        );
      ...
      console.warn(
        `[html2pptx] object-fit image fallback (${el.src}): ${err.message}`,
      );
    }
  });
...
async function getBodyDimensions(page) {
  ...
  if (
    widthOverflowPt > OVERFLOW_TOLERANCE_PT ||
    heightOverflowPt > OVERFLOW_TOLERANCE_PT
  ) {
    const directions = [];
...
async function addBackground(slideData, targetSlide, pres, tmpDir) {
  if (slideData.background.type === "image" && slideData.background.path) {
    ...
    try {
      ...
      console.warn(
        `[html2pptx] background image failed (${imagePath}): ${err.message}`,
      );
...
function pickShapeKind(pres, shapeMeta) {
  ...
  if (shapeMeta.rectRadius && shapeMeta.rectRadius > 0)
...
function addElements(slideData, targetSlide, pres, tmpDir) {
  ...
  const sortedElements = slideData.elements
    .map((el, idx) => ({
      el,
      idx,
      z: Number.isFinite(el.zIndex) ? el.zIndex : 0,
    }))
  ...
  for (const el of sortedElements) {
    if (el.type === "image") {
      ...
      if (el._skipRender) {
        if (el._skipReason)
          console.warn(`[html2pptx] image skipped — ${el._skipReason}`);
        continue;
      }
      let imagePath =
        el.src && el.src.startsWith("file://")
          ? el.src.replace("file://", "")
          : el.src;
      ...
      try {
        ...
        if (isDataUrl) opts.data = imagePath;
          ...
          opts.path = isLocalFile
            ? fixImageExtension(imagePath, tmpDir)
            : imagePath;
        targetSlide.addImage(opts);
      } catch (err) {
        console.warn(
          `[html2pptx] failed to add image ${imagePath}: ${err.message}`,
        );
      }
      ...
      let adjustedH =
        el.position.h * (1 + COMPENSATION.LIST_HEIGHT) +
      ...
      if (!isRotatedText && adjustedW > MAX_TEXT_WIDTH_IN)
        adjustedW = MAX_TEXT_WIDTH_IN;
      ...
      if (!hasContainerPadding || iconOnlyText || zeroTextInset)
        textOptions.inset = 0;
      ...
      if (
        !el.style?._allowWrap &&
        (el.noWrap || isShortAutoNoWrapText(el) || isHeadingWithBreaks)
      )
...
async function extractSlideData(page, slideDims) {
  return await page.evaluate((slideDims) => {
    ...
    const bodyWPx =
      slideRootRect.width ||
      slideRoot.scrollWidth ||
      document.body.clientWidth ||
      1280;
    const bodyHPx =
      slideRootRect.height ||
      slideRoot.scrollHeight ||
    ...
    const heuristicMapFont = (font, cjkText) => {
      ...
      if (
        /(condensed|narrow|compressed|compact|extended|cond\b|cn\b)/.test(
          cleaned,
        )
      ) {
      ...
      if (
        /(slab|rockwell|chunk|courier prime|josefin slab|zilla slab|alfa)/.test(
          cleaned,
        )
      ) {
        return "Tinos";
      ...
      if (
        /(pixel|pixelify|press start|vt323|silkscreen|jersey|retro|8.?bit|terminal)/.test(
          cleaned,
        )
      ) {
    ...
    const mapFontFace = (fontFamily, textContent = "") => {
      ...
      for (const font of fonts) {
        ...
        if (guessed) {
          const guessedIsCJK = CJK_FONT_FRAGMENTS.some((f) =>
            guessed.toLowerCase().includes(f),
          );
    ...
    const isIconOnlyText = (text = "") => {
      ...
      const compact = t.replace(/[\s\uFE0E\uFE0F\u200D\u20E3]/g, "");
      if (!compact || Array.from(compact).length > 4) return false;
      if (hasCJKChars(compact) || /[A-Za-z0-9０-９]/.test(compact))
        return false;
    ...
    const appendTextRuns = (runs, text, baseOptions = {}) => {
      ...
      const shouldSplit =
        (hasCJKChars(fullText) &&
          (hasLatinChars(fullText) ||
            hasDigitChars(fullText) ||
      ...
      while ((match = tokenRe.exec(fullText)) !== null) {
        if (match.index > last)
          pushTextRun(runs, fullText.slice(last, match.index), baseOptions);
        const token = match[0];
      ...
      if (last < fullText.length)
        pushTextRun(runs, fullText.slice(last), baseOptions);
    };
    ...
    const splitTextIntoFontRuns = (text, baseOptions = {}) => {
      ...
      return runs.length > 1 ||
        (runs[0] && !sameRunOptions(runs[0].options, baseOptions))
        ? runs
        : null;
    };
    ...
    const computeTransparency = (colorStr, opacityStr) => {
      ...
      const opacityT =
        Number.isFinite(cssOpacity) && cssOpacity < 1
        ...
        if (isText) {
          ...
          if (
            ...
            const parentIsRowFlex =
              ...
              !String(parentComputed.flexDirection || "row").startsWith(
                "column",
              );
            const isBlockSpan =
              !parentIsRowFlex &&
            ...
            if (
              isMediumOrBold(computed.fontWeight) &&
              shouldUseEmphasisFont(node.textContent)
            )
            ...
            if (isBlockSpan) {
              ...
              if (lastRun && !/\s$/.test(lastRun.text))
                appendTextRuns(runs, " ", baseOptions);
            }
          } else {
    ...
    const resolveIconGlyph = (el) => {
      ...
      const guessed =
        inferIconByKeyword(iconName || raw) || inferIconByKeyword(cls);
    ...
    document.querySelectorAll("h1,h2,h3,h4,h5,h6,p").forEach((parent) => {
      ...
      if (
        blockTextChildren.length < 2 ||
        blockTextChildren.length !== textBearingChildren.length
      )
        return;
    ...
    document.querySelectorAll("*").forEach((parent) => {
      ...
      const blockTextChildren = Array.from(parent.children).filter(
        (c) =>
          INLINE_TEXT_CHILD_TAGS.includes(c.tagName) &&
          c.textContent.trim() &&
          _isBlockDisplay(c),
      );
    ...
    const getNearestTextClipBounds = (el) => {
      ...
      while (p && p !== document.body && p !== document.documentElement) {
        ...
        if (hasBg || hasBgImage || hasBorder || hasShadow || hasRadius) {
          ...
          const w = Math.max(
            1,
            r.width - borders[1] - borders[3] - padL - padR,
          );
          const h = Math.max(
            1,
            r.height - borders[0] - borders[2] - padT - padB,
          );
          return {
            x: pxToInch(relX(x)),
            y: pxToInch(relY(y)),
    ...
    const getFlexTextRightBound = (el, rect) => {
      ...
      const nextRightSibling = siblingRects
        .filter(
          (r) =>
            r.left >= rect.right - 1 || r.left > rect.left + rect.width * 0.5,
        )
    ...
    const getNextFlowTextTopBound = (el, rect) => {
      ...
      for (let sib = el.nextElementSibling; sib; sib = sib.nextElementSibling) {
        ...
        const txt = normalizeWhitespaceForPpt(
          sib.textContent || "",
          scs.whiteSpace,
        ).trim();
        if (!txt) continue;
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      if (el._isIcon) {
        ...
        const parentComputed = iconParent
          ? window.getComputedStyle(iconParent)
          : null;
        const parentRect = iconParent
          ? iconParent.getBoundingClientRect()
          : null;
        const textBearingSiblings = iconParent
          ? Array.from(iconParent.children).filter(
              (c) => c !== el && (c.textContent || "").trim(),
            )
          : [];
        ...
        if (
          ...
          const fsPx =
            ...
            Math.min(parentRect.width, parentRect.height);
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          const glyphFontSize = Math.max(
            8,
            Math.min(fsPx * PT_PER_PX * SCALE * 1.25, 48),
          );
          elements.push({
            type: "p",
            text: el._iconGlyph,
      ...
      const isPlaceholderElement =
        ...
        classTokens.some((cls) =>
          /^(chart|plot|viz|graph)-?placeholder$/i.test(cls),
        );
      ...
      if (
        ...
        if (rect.width > 0 && rect.height > 0) {
          try {
            ...
            const cloneNodes = [
              svgEl,
              ...Array.from(svgEl.querySelectorAll("*")),
            ];
      ...
      if (el.tagName === "TABLE") {
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          for (const col of Array.from(el.querySelectorAll("colgroup col"))) {
            ...
            const styleWidth =
              parseFloat(col.style.width || colComputed.width) || 0;
            const rectWidth = col.getBoundingClientRect
              ? col.getBoundingClientRect().width
              : 0;
            const widthPx = rectWidth || styleWidth;
            if (widthPx > 0) {
              for (let i = 0; i < span; i++)
                colGroupWidths.push(pxToInch(widthPx / span));
            }
          }
          let colWidths =
            colGroupWidths.length > 0 ? colGroupWidths : undefined;
          if (!colWidths) {
            let bestCells = [];
            ...
            for (const tr of trList) {
              ...
              const expandedCount = cells.reduce(
                (sum, c) => sum + getSpan(c, "colspan"),
                0,
              );
              if (
                expandedCount > bestExpandedCount ||
                (expandedCount === bestExpandedCount &&
                  cells.length > bestCells.length)
              ) {
                bestCells = cells;
            ...
            if (bestCells.length > 0) {
              ...
              for (const cell of bestCells) {
                ...
                const width =
                  pxToInch(cell.getBoundingClientRect().width) / span;
          ...
          for (const tr of trList) {
            ...
            const rowOut = cells.map((cell) => {
              ...
              const cellOpts = {
                ...
                fontFace: mapFontFace(
                  cellComputed.fontFamily,
                  cell.textContent,
                ),
          ...
          if (rows.length > 0) {
            ...
            const borderColor =
              tableComputed.borderTopColor &&
              tableComputed.borderTopColor !== "rgba(0, 0, 0, 0)"
                ? rgbToHex(tableComputed.borderTopColor)
                : "CCCCCC";
            const borderPt = Math.max(
              0.25,
              pxToPoints(tableComputed.borderTopWidth) || 0.5,
            );
            elements.push({
              type: "table",
              rows,
      ...
      const isContainer =
        blockContainerTags.has(el.tagName) || isStandaloneInlineShape;
      if (isContainer) {
        const computed = window.getComputedStyle(el);
        ...
        if (!isTextClippedBackground && bgImage && bgImage !== "none") {
          ...
          const isOtherGradient =
            /^(repeating-)?(linear|radial|conic)-gradient/.test(bgImage) &&
            !isLinear;
          // Collect all url() entries from possibly multi-layer backgroundImage
          const urlMatches = [
            ...bgImage.matchAll(/url\(["']?([^"')]+)["']?\)/g),
          ].map((m) => m[1]);
          ...
          if (
            ...
            const gradientOpacityT = computeTransparency(
              null,
              computed.opacity,
            );
            if (gradientOpacityT !== null)
              gradientImg.transparency = gradientOpacityT;
        ...
        const hasUniformBorder =
          hasExportableBorder && borders.every((b) => b === borders[0]);
        const borderLines = [];
        ...
        if (hasExportableBorder && !hasUniformBorder) {
          ...
          const styleToDash = (s) =>
            s === "dashed" ? "dash" : s === "dotted" ? "dot" : "solid";
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (hasBg || hasUniformBorder) {
              ...
              if (gradientFill) {
                ...
                fillTransparency = computeTransparency(
                  bgFillValue,
                  computed.opacity,
                );
              ...
              const shapeKind =
                isAlmostSquare && isFullRound ? "ellipse" : "rect";
              ...
              if (isShortPaddedBadgeShape) {
                const extraX = Math.min(
                  0.025,
                  Math.max(0.012, shapePosition.w * 0.04),
                );
                const extraY = Math.min(
                  0.014,
                  Math.max(0.006, shapePosition.h * 0.06),
                );
                shapePosition = {
              ...
              if (lineSpec && !isShortPaddedBadgeShape) {
                ...
                const insetX = Math.max(
                  0,
                  Math.min(strokeInset, shapePosition.w / 2 - 0.001),
                );
                const insetY = Math.max(
                  0,
                  Math.min(strokeInset, shapePosition.h / 2 - 0.001),
                );
              ...
              const containerRotation = getRotation(
                computed.transform,
              ...
              const cssBoxH =
                el.offsetHeight || parseFloat(computed.height) || 0;
              ...
              if (isThinRotatedFillBox) {
                const matrixMatch =
                  computed.transform.match(/matrix\(([^)]+)\)/);
                const vals = matrixMatch
                  ? matrixMatch[1].split(",").map((v) => parseFloat(v.trim()))
                  : null;
                const angleRad = (containerRotation * Math.PI) / 180;
                const a =
                  vals && Number.isFinite(vals[0])
                    ? vals[0]
                    : Math.cos(angleRad);
                const b =
                  vals && Number.isFinite(vals[1])
                    ? vals[1]
                    : Math.sin(angleRad);
                const c =
                  vals && Number.isFinite(vals[2])
                    ? vals[2]
                    : -Math.sin(angleRad);
                const d =
                  vals && Number.isFinite(vals[3])
                    ? vals[3]
                    : Math.cos(angleRad);
                const originParts = String(
                  computed.transformOrigin || "0 0",
                ).split(/\s+/);
                ...
                const lineEl = {
                  ...
                  width: Math.max(
                    0.5,
                    pxToPoints(String(Math.max(1, cssBoxH)) + "px"),
                  ),
            ...
            const hasBlockTextChild = Array.from(el.children).some(
              (c) =>
                BLOCK_TEXT_CHILD_TAGS.has(c.tagName) && c.textContent.trim(),
            );
            ...
            const shouldClaimText =
              !hasBlockTextChild && (directText || !hasInlineTextChild);
      ...
      if (el.tagName === "UL" || el.tagName === "OL") {
        ...
        const isFlexColumnList =
          (ulComputed.display === "flex" ||
            ulComputed.display === "inline-flex") &&
          String(ulComputed.flexDirection || "").startsWith("column");
        const flexListGapPt = isFlexColumnList
          ? Math.max(
              cssLengthToPoints(ulComputed.rowGap),
              cssLengthToPoints(ulComputed.gap),
            )
          : 0;
        ...
        if (!isOrdered && liElements.length > 0) {
          ...
          if (listStyleNone) {
            ...
            if (hasBefore) {
              ...
              const liFs =
                pxToFontSize(liStyle.fontSize, firstLi.textContent) || 12;
              customBullet = {
        ...
        liElements.forEach((li, idx) => {
          ...
          const liIsFlex =
            liComputed.display === "flex" ||
            liComputed.display === "inline-flex";
          const runs = liIsFlex
            ? [
                {
          ...
          taLi === "start"
            ...
            : taLi === "end"
              ...
              : taLi === "left" ||
                ...
                ? taLi
                : "left";
        const listTextPaint = resolveTextPaintStyle(
          computed,
          liElements[0] || el,
        );
      ...
      let rect = el.getBoundingClientRect();
      const computed = window.getComputedStyle(el);
      let text = normalizeWhitespaceForPpt(
        el.textContent,
        computed.whiteSpace,
      ).trim();
      ...
      const isVerticalWM =
        computed.writingMode === "vertical-rl" ||
        computed.writingMode === "vertical-lr";
      ...
      const isFlexRowTitleText =
        ...
        !el.querySelector("br") &&
        (isMediumOrBold(computed.fontWeight) ||
          pxToFontSize(computed.fontSize, text) >= 8.5);
      const noWrap =
      ...
      baseStyle._fontSizeClampRatio = fontSizeClampRatio(
        computed.fontSize,
        text,
      );
      ...
      if (nextFlowTextTopBound !== null)
        baseStyle._nextTextTopBound = nextFlowTextTopBound;
      if (flexTextRightBound !== null)
        baseStyle._preferredRightBound = flexTextRightBound;
      const isShortPaddedBadgeText =
        baseStyle._hasContainerPadding &&
        text.length <= 24 &&
        rect.height <= 48 &&
        (computed.whiteSpace === "nowrap" ||
          (parseFloat(computed.borderRadius) || 0) > 0);
      ...
      if (textPaint.transparency !== null)
        baseStyle.transparency = textPaint.transparency;
      ...
      const pushInlineBackgroundShapes = () => {
        const inlineNodes = Array.from(
          el.querySelectorAll(
            "span,mark,b,strong,i,em,u,a,code,small,big,label",
          ),
        );
        for (const node of inlineNodes) {
          ...
          const hasNodeBg =
            nodeBg && nodeBg !== "rgba(0, 0, 0, 0)" && nodeBg !== "transparent";
          ...
          if (radiusValue > 0) {
            if (radius.includes("%"))
              rectRadius =
                (Math.min(nodeRect.width, nodeRect.height) * radiusValue) /
                100 /
                PX_PER_IN;
      ...
      if (textIsIconOnly && baseStyle._centerInFrame && rotation === null) {
        const fsPx =
          parseFloat(computed.fontSize) || Math.min(rect.width, rect.height);
        const lineHeightPx =
          computed.lineHeight && computed.lineHeight !== "normal"
      ...
      if (hasFormatting) {
        ...
        const baseRunOptions = {
          ...
          bold:
            shouldApplyPptBold(computed.fontWeight) &&
        ...
        if (
          isMediumOrBold(computed.fontWeight) &&
          shouldUseEmphasisFont(transformedText)
        )
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    try {
        ...
        await page.evaluate(async () => {
          ...
          const tailwindLooksActive = () => {
            ...
            for (const el of nodes) {
              ...
              if (tokens.includes("absolute") && cs.position === "absolute")
                return true;
              if (tokens.includes("flex") && cs.display === "flex") return true;
              if (tokens.includes("grid") && cs.display === "grid") return true;
              if (tokens.some((t) => /^top-\[/.test(t)) && cs.top !== "auto")
                return true;
              if (
                tokens.some((t) => /^right-\[/.test(t)) &&
                cs.right !== "auto"
              )
                return true;
              if (
                tokens.includes("-translate-y-1/2") &&
                cs.transform &&
                cs.transform !== "none"
              )
                return true;
      ...
      const slideWIn = pres.presLayout
        ? pres.presLayout.width / EMU_PER_IN
        : 10;
      const slideHIn = pres.presLayout
        ? pres.presLayout.height / EMU_PER_IN
        : 5.625;
