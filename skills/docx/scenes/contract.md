...
function selectContractType(keywords, topic) {
  ...
  if (/framework|strategic|cooperation agreement/.test(keywords))
    return "framework";
  if (/terms|platform rules|user agreement|privacy/.test(keywords))
    return "terms";
    ...
    rows: fields.map((label, i) => {
      ...
      const bVal =
        i === fields.length - 1
          ...
          : safeText(partyB?.[i], "");
      const displayA =
        i === 0 ? `Party A (甲方): ${aVal}` : `${label}: ${aVal}`;
      const displayB =
        i === 0 ? `Party B (乙方): ${bVal}` : `${label}: ${bVal}`;
      return new TableRow({
        children: [
          new TableCell({
            ...
            children: [
              new Paragraph({
                children: [
                  new TextRun({ text: displayA, size: 24, color: "000000" }),
                ],
              }),
            ],
          }),
          new TableCell({
            ...
            children: [
              new Paragraph({
                children: [
                  new TextRun({ text: displayB, size: 24, color: "000000" }),
                ],
              }),
            ],
          }),
