...
function selectCoverRecipe(docType, industry, titleLength) {
  ...
  if (docType === "proposal_report")
    return { recipe: "R5", palette: "ACADEMIC" };
  ...
  if (docType === "proposal" || docType === "plan")
    return { recipe: "R4", palette: "GO-1" };
...
new Paragraph({
  ...
  border: {
    top: { style: BorderStyle.SINGLE, size: 18, color: P.accent, space: 20 },
  },
  children: [],
});
...
function calcCoverSpacing(params) {
  ...
  const topSpacing = Math.max(
    rawTop - Math.max(0, FOOTER_MIN - rawBottom),
    400,
  );
...
function buildCoverR1(config) {
  ...
  const { titlePt, titleLines } = calcTitleLayout(
    config.title,
    availableWidth,
    40,
    24,
  ...
  children.push(
    new Paragraph({
      indent: { left: padL, right: padR },
      border: {
        top: { style: BorderStyle.SINGLE, size: 2, color: P.accent, space: 8 },
      },
      spacing: { before: 200 },
      children: [
        new TextRun({
...
function buildCoverR4(config) {
  ...
  const topSpacing = Math.max(
    UPPER_H - contentEstimate - spacerIntrinsic - 800,
    400,
  );
...
function calcR5MetaLayout(metaEntries, fontPt = 12) {
  ...
  const tablePct = Math.min(
    75,
    Math.max(55, Math.ceil((totalNeedTw / 11906) * 100)),
  );
  // Label % within the table, clamped to 25–45%
  const rawLabelPct = Math.ceil(
    (labelNeedTw / ((tablePct / 100) * 11906)) * 100,
  );
...
function buildCoverR5(config) {
  ...
  const { titlePt, titleLines } = calcTitleLayoutMixed(
    config.title,
    contentW,
    36,
    24,
  ...
  const fixedH =
    schoolNameH + titleTotalH + subtitleH + metaTableH + footerH + spacerParas;
  ...
  const midSpacing = Math.min(
    Math.floor((remaining - simMarginT) * 0.18),
    2000,
  );
  const bottomSpacing = Math.min(
    remaining - topSpacing + simMarginT - midSpacing,
    5500,
  );
