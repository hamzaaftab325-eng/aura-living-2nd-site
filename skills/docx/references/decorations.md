...
function buildCoverR6(config) {
  ...
  const { titlePt, titleLines } = calcTitleLayoutR6(
    config.title,
    availW,
    36,
    22,
  ...
  children.push(
    new Paragraph({ indent: ind, spacing: { before: topSpacing } }),
  );
  ...
  for (const line of config.metaLines || []) {
    ...
    const sep =
      line.indexOf("：") !== -1 ? "：" : line.indexOf(":") !== -1 ? ":" : null;
  ...
  const flSpacing =
    footerLeft.length > 60 ? 5 : footerLeft.length > 40 ? 10 : 20;
        ...
        children: [
          new TableCell({
            ...
            children: [
              new Paragraph({
                children: [
                  new TextRun({ text: label, size: 21, color: "666666" }),
                ],
              }),
            ],
          }),
