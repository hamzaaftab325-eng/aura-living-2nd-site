...
function loadPlaywright() {
  ...
  throw new Error(
    "Playwright not found. Install: npm install -g playwright@1.50.0",
  );
}
...
async function preRenderHooks(page) {
  ...
  if (vOverflowFix.fixes.length) {
    ...
    vOverflowFix.fixes.forEach((f) => {
      console.log(
        `    ${f.selector}: ${f.clipped}px clipped → overflow:visible`,
      );
    });
  }
...
async function convert(inputFile, outputFile, customCSS, options = {}) {
  ...
  console.log(
    `   Engine: Playwright + ${noPaged ? "Chromium native @page" : "Paged.js"}`,
  );
  ...
  try {
    ...
    if (continuousInfo) {
      ...
      try {
        ...
        console.log(
          "  ⚠ Paged.js did not create .pagedjs_page elements (timeout 30s)",
        );
        console.log("    Falling back to Chromium native @page pagination.");
        warnings.push(
          "Paged.js rendering timed out; fell back to native pagination",
        );
      }
      ...
      const bgFix = await page.evaluate(() => {
        ...
        const pageBg = !isTransparent(bodyBg)
          ? bodyBg
      ...
      if (bgFix.pagesFixed > 0) {
        console.log(
          `  ✓ Background fix: ${bgFix.pagesFixed} pages → ${bgFix.pageBg}`,
        );
      }
    ...
    if (
      mergeFiles &&
      mergeFiles.length &&
      postResult.pageCount > postResult.originalPages
    ) {
      console.log(
