...
async function main() {
  const { input, width, height, minGap, noTextOverlap } = parseArgs(
    process.argv,
  );
  ...
  console.log(
    `   Pass 2 (text↔text): ${noTextOverlap ? "DISABLED" : "enabled"}`,
  );
  ...
  try {
    ...
    const layer3Text = result.textElements_by_layer
      ? result.textElements_by_layer.layer3
      : "?";
    const layer1Text = result.textElements_by_layer
      ? result.textElements_by_layer.layer1
      : "?";
    console.log(
    ...
    if (!noTextOverlap) {
      ...
      if (textOverlaps.overlaps.length > 0) {
        ...
        allIssues.push(
          ...textOverlaps.overlaps.map((o) => ({ pass: 2, ...o })),
        );
      } else {
        console.log(
